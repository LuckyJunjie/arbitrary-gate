...
| UI-02 | 宣纸纹理背景+牙色色系 | P0 | ✅ 已完成 | App.vue 固定层含宣纸 SVG 纹理（#f5efe0 牙色）；所有视图根元素背景改为 transparent，宣纸纹理全局一致显示；ShareView/BookshelfView/CardsView/ShopView/ManuscriptView 均已修复 |
...
| D-02 | 历史事件卡池数据（600+） | P0 | ✅ 完成 | 实际导入 600 条（EV001-EV600），覆盖先秦至近代+虚构传说+神话武侠；SQL 文件：`docker/mysql/init/02-event_cards.sql` |
...
1. **P0 已全部实现（54/54）**：抽卡/故事/认证/AI安全检测全部完成，微信登录/事件卡池/关键词卡池已就绪
2. **P1 已全部实现（34/34）**：所有核心体验功能（入局三问/手势/涟漪/卷轴/手稿等）均已完成
3. **P2 已全部实现（20/20）**：分享/充值/山河图/ServiceWorker/CI均已完成
4. **P3 已全部实现（15/15）**：AI画师/音效/墨晶衰减/批注彩蛋等体验增强功能全部完成
5. **U-05 墨晶充值**：后端业务逻辑完整，已添加微信 API v3 HTTP 调用；生产环境需配置 WX_PAY_MCH_ID + WX_PAY_API_KEY +商户证书
