...
public class AiPhraseFilter {
    ...
    /**
     * deleteWords：直接删除的词（功能性过渡/连接词 + AI腔词补充2026-04-10）
     */
    private final Set<String> deleteWords;

    public AiPhraseFilter() {
        this.deleteWords = Set.of(
                ...
                "如此", "这般",
                // AI腔词补充（2026-04-10）
                "缓缓道来", "不禁微微一怔", "眼中闪过", "轻轻叹了口气"
        );
    }

    /**
     * AI-06: 初始化黑名单（@PostConstruct）
     */
    @PostConstruct
    public void init() {
        ...
        List<String> sorted = this.blacklist.stream()
                .sorted((a, b) -> Integer.compare(b.length(), a.length()))
                .collect(Collectors.toList());

        this.patterns = sorted.stream()
        ...
        log.info("[AI-06] AiPhraseFilter 初始化完成，黑名单词数: {}", this.blacklist.size());
    }
    ...
    public String filter(String text) {
        ...
        for (Pattern pattern : patterns) {
            result = pattern.matcher(result).replaceAll(getReplacement(pattern.pattern()));
        }
        return result;
    }
     ...
     * @return 黑名单词列表（去重）
     */
    public List<String> getBlockedPhrases(String text) {
        if (text == null || text.isBlank()) {
     ...
     * 优先保留原文语义，移除腔调词
     */
    private String getReplacement(String matchedWord) {
        if (deleteWords.contains(matchedWord)) {
            return "";
        }
        return "";
    }
